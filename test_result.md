...
user_problem_statement: "Run a SHORT targeted backend verification only for Track 27.11A preview endpoints. Base URL: https://backup-forensics.preview.emergentagent.com. Login via POST /api/auth/multi-login with jaymn.judd@mascigc.com / Maddix123! and use portal_tokens.admin as X-Admin-Token. Verify only these endpoints: /api/health/full, /api/admin/recovery/snapshot, /api/admin/backups-scheduler-state, /api/admin/backups/integrity-check, /api/admin/production-certification, /api/version. Check for exact contracts relevant to Track 27.11A: scheduler truth agreement, recent five-backup lineage fields, release-scoped certification fields, version build/process-start semantics."
...
test_plan:
  current_focus:
    - "TRACK 27.11A - Recovery/Bundle Backend API Endpoints"
  stuck_tasks: []
  test_all: false
