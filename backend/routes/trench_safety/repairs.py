    ...
    *,
    ...
    async def open_repair(
        ...
        return await with_idempotency(
            db, key, actor if isinstance(actor, dict) else {"role": "shop_or_admin"},
            _do_create, workflow="trench_repair_open",
        )
    ...
    async def update_repair(
        ...
        # TRACK 22.4b-followup-Trench-Writes-Idempotency · exactly-once.
        # Notes are $push-appended so retries would double-append without
        # this reservation lock.
        from lib.idempotency import with_idempotency, idem_key_from_request  # noqa: PLC0415
        key = idem_key_from_request(request)
        ...
        return await with_idempotency(
            db, key, actor if isinstance(actor, dict) else {"role": "shop_or_admin"},
            _do_create, workflow="trench_repair_update",
        )
    ...
    async def complete_repair(
        ...
        # TRACK 22.4b-followup-Trench-Writes-Idempotency · exactly-once.
        # Preserves B-04 invariant: "Repair Complete ≠ Safe To Use".
        # This handler transitions the repair to Completed and, when
        # required, opens an Inspection Hold that Safety must clear
        # via /verify. The idempotency wrap ensures a retry never
        # re-opens duplicate Inspection Holds or double-clears the
        # Maintenance Hold on the same reservation.
        from lib.idempotency import with_idempotency, idem_key_from_request  # noqa: PLC0415
        key = idem_key_from_request(request)
        ...
        async def _do_create():
            ...
            # (B-04: Shop cannot flip the asset to Safe To Use.)
        ...
        return await with_idempotency(
            db, key, actor if isinstance(actor, dict) else {"role": "shop_or_admin"},
            _do_create, workflow="trench_repair_complete",
        )
