"""
hr_users.py — DB-backed HR user roster with per-user passwords.

Mirrors `project_managers.py` so admins can add/remove HR personnel
(HR coordinators, HR managers) and issue per-person
...
Schema (db.hr_users):
  ...
  phone                str (optional)
  role                 str  e.g. "HR Manager", "HR Coordinator", "Parts"
  is_active            bool
  disabled             bool
...
def _normalize(doc: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "role": (doc.get("role") or "HR Coordinator").strip(),
...
async def seed_hr_users(db) -> None:
    ...
    try:
        await db.hr_users.create_index("email", unique=True)
    except Exception as e:  # noqa: BLE001
        logger.warning(f"hr_users index: {e}")
    if await db.hr_users.count_documents({}) > 0:
        return
    docs = [_normalize(r) for r in INITIAL_HR_USERS]
    if docs:
...
def _hr_session_epoch() -> str:
    return os.environ.get("ADMIN_SESSION_EPOCH", "1").strip() or "1"


def make_hr_user_token(user_id: str, password_hash: str) -> str:
    ...
    msg = f"epoch={_hr_session_epoch()}|hr_user:{user_id}:{password_hash[:16]}".encode()
    sig = hmac.new(_pm_hmac_secret(), msg, hashlib.sha256).hexdigest()
    return f"{user_id}.{sig}"


def parse_hr_user_token(token: str) -> Optional[Tuple[str, str]]:
...
async def is_valid_hr_user_token_async(db, token: str) -> Optional[dict]:
    parsed = parse_hr_user_token(token)
    if not parsed:
        return None
    user_id, _ = parsed
    user = await db.hr_users.find_one({"id": user_id}, {"_id": 0})
    if not user or user.get("disabled"):
        return None
    pwh = user.get("password_hash") or ""
    if not pwh:
        return None
    expected = make_hr_user_token(user_id, pwh)
...
async def list_hr_users(db, only_active: bool = False) -> List[Dict[str, Any]]:
    q: Dict[str, Any] = {"is_active": True} if only_active else {}
    cursor = db.hr_users.find(q, {"_id": 0}).sort("name", 1)
    return await cursor.to_list(500)


async def add_hr_user(db, body: Dict[str, Any]) -> Dict[str, Any]:
    doc = _normalize(body)
    if not doc["name"]:
        raise ValueError("name is required")
    if not doc["email"] or "@" not in doc["email"]:
        raise ValueError("a valid email is required")
    if await db.hr_users.find_one({"email": doc["email"]}, {"_id": 0}):
        raise ValueError(f"A HR user with email {doc['email']} already exists")
    await db.hr_users.insert_one(doc)
    doc.pop("_id", None)
    return doc


async def update_hr_user(db, user_id: str, body: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    if "email" in body:
        ...
        if not new_email or "@" not in new_email:
            raise ValueError("a valid email is required")
        clash = await db.hr_users.find_one(
            {"email": new_email, "id": {"$ne": user_id}}, {"_id": 0}
        )
        if clash:
            raise ValueError(f"Another HR user already uses {new_email}")
        update_fields["email"] = new_email
    if "phone" in body:
        update_fields["phone"] = (body.get("phone") or "").strip()
    if "role" in body:
        update_fields["role"] = (body.get("role") or "HR Coordinator").strip()
    if "is_active" in body:
        update_fields["is_active"] = bool(body["is_active"])
    if "disabled" in body:
        update_fields["disabled"] = bool(body["disabled"])
    res = await db.hr_users.update_one({"id": user_id}, {"$set": update_fields})
    if res.matched_count == 0:
        return None
    return await db.hr_users.find_one({"id": user_id}, {"_id": 0})


async def delete_hr_user(db, user_id: str) -> bool:
    res = await db.hr_users.delete_one({"id": user_id})
    return res.deleted_count > 0


async def find_hr_user_by_email(db, email: str) -> Optional[dict]:
    if not email:
        return None
    return await db.hr_users.find_one(
        {"email": email.strip().lower()}, {"_id": 0}
    )


async def set_hr_user_password(
...
async def stamp_hr_login(db, user_id: str, ip: Optional[str] = None) -> None:
    fields: dict = {"last_login_at": _now()}
    if ip:
        fields["last_login_ip"] = ip
    await db.hr_users.update_one({"id": user_id}, {"$set": fields})
...
_HR_RESET_TOKEN_TTL_SECONDS = 30 * 60  # 30 minutes


def make_hr_reset_token(user_id: str, password_hash: str) -> str:
    ...
    if not user_id or not password_hash:
        raise ValueError("user_id and password_hash required")
    exp = int(_time.time()) + _HR_RESET_TOKEN_TTL_SECONDS
    msg = f"reset|exp={exp}|hr_user:{user_id}:{password_hash[:16]}".encode()
    sig = hmac.new(_pm_hmac_secret(), msg, hashlib.sha256).hexdigest()
    return f"{exp}.{user_id}.{sig}"


async def consume_hr_reset_token(db, token: str) -> Optional[dict]:
    """Validate a forgot-password token. Returns the HR user doc if
    ...
    if not pwh:
        return None
    msg = f"reset|exp={exp}|hr_user:{user_id}:{pwh[:16]}".encode()
...
def public_hr_user_view(user: dict) -> dict:
    if not user:
        return {}
...
__all__ = [
    ...
    "update_hr_user",
    "delete_hr_user",
    "find_hr_user_by_email",
    "set_hr_user_password",
    "stamp_hr_login",
    "make_hr_user_token",
    "is_valid_hr_user_token_async",
    "make_hr_reset_token",
    "consume_hr_reset_token",
    "public_hr_user_view",
    "verify_password",
    "generate_temp_password",
]
