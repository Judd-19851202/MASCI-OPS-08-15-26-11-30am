...
def extract_draft_photos(draft: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    def _add(entry: Any, ref: str, source: str, caption: str = "") -> None:
        b64 = _b64_of(entry)
        if not b64:
            return
...
async def analyze_draft_photos(db, *, report_id: str, draft: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    try:
        photos = extract_draft_photos(draft)
    except Exception:  # noqa: BLE001
        return []
    if not photos:
        return []
    ...
    for p in photos:
        ...
        if cached and cached.get("envelope"):
            results[p["ref"]] = _to_observation(p, cached["envelope"])
        else:
            pending.append(p)
    ...
    if pending:
        ...
        gathered = await asyncio.gather(*[_one(p) for p in pending], return_exceptions=True)
        for item in gathered:
            ...
            try:
                ...
                pass
            results[p["ref"]] = _to_observation(p, envelope)

    return [results[p["ref"]] for p in photos if p["ref"] in results]
