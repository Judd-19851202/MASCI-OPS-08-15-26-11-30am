...
# ──────────────────────────────────────────────────────────────────────
# PM is operationally responsible for crews on their projects but has
# had ZERO compliance visibility (training currency, PPE issuance,
# CAPA exposure) — they can see daily reports and inspections but not
# the compliance signal needed to safely assign work. These endpoints
# give PM scoped READ-ONLY visibility scoped to "every employee whose
# name has appeared on a daily report under a PM-assigned project in
# the last 180 days" (operator-explicit project-scoped crew definition).
#
# NO Safety governance authority. NO HR write authority. NO delete.
# ════════════════════════════════════════════════════════════════════════

async def _pm_crew_employee_names(actor, days: int = 180) -> List[str]:
    ...
    # New per-PM token doc — find assigned projects.
    pm_email = (actor.get("email") or "").lower()
    if not pm_email:
        return []
    # Find projects where the PM is listed.
    proj_names: List[str] = []
    ...
    async for r in db.daily_reports.find(
        ...
        # daily_reports schema has varied historically — sample
        # every common key for crew names.
...
async def pm_crew_training_records(
    actor=Depends(require_admin_async),
    limit: int = Query(default=200, ge=1, le=500),
):
    """iter353e · PM scoped training records for crew on PM's projects."""
    names = await _pm_crew_employee_names(actor)
    q: Dict[str, Any] = {}
...
async def pm_crew_summary(actor=Depends(require_admin_async)):
...
@app.post("/api/legacy-imports/upload")
async def li_upload(
    request: Request,
    file: UploadFile = File(...),
    ...
    if len(data) > 25 * 1024 * 1024:  # 25 MB hard cap (operator can revise)
    ...
    # Reuse photo_storage's key builder pattern but namespace under legacy-imports/
    import datetime as _dt  # noqa: PLC0415
    today = _dt.datetime.now(_dt.timezone.utc)
    safe_src = "".join(c if c.isalnum() or c in "-_." else "_" for c in (actor["actor_id"] or "unknown"))
    key = f"legacy-imports/{today:%Y/%m}/{batch_id or 'solo'}/{safe_src}/{uuid.uuid4().hex}.{ext}"
    ...
    row = {
        ...
        "matches": {
            ...
            "equipment": {"suggested_id": None, "suggested_name": None,
                          "confidence": 0.0, "alternatives": []},
            "project": {"suggested_id": None, "suggested_name": None, "confidence": 0.0,
                        "alternatives": []},
            "duplicate_of": None,
        },
        "review": {
...
async def li_list(
    ...
    if batch_id:
        q["batch_id"] = batch_id
    cursor = db.legacy_imports.find(q, {"_id": 0}).sort("created_at", -1).limit(max(1, min(500, limit)))
    items = await cursor.to_list(None)
    return {"count": len(items), "items": items}


@app.get("/api/legacy-imports/{import_id}")
...
@app.get("/api/legacy-imports/{import_id}/file")
...
@app.patch("/api/legacy-imports/{import_id}")
...
@app.post("/api/legacy-imports/{import_id}/approve")
async def li_approve(
    ...
    # Scope check before calling helper
    q = {"id": import_id, **_li_scope_filter(actor)}
    doc = await db.legacy_imports.find_one(q, {"_id": 0})
    if not doc:
        raise HTTPException(404, "import not found")
...
@app.post("/api/legacy-imports/{import_id}/reject")
async def li_reject(
    import_id: str,
    body: _LiReject,
    actor=Depends(_li_require_uploader),
...
async def li_admin_audit(
    ...
    cursor = db.legacy_import_audit.find(q, {"_id": 0}).sort("timestamp", -1).limit(max(1, min(1000, limit)))
    items = await cursor.to_list(None)
    return {"count": len(items), "items": items}
