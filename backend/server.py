...
# ============================================================
# Shop Activity Feed — flatten every shop_signoff entry across
# every equipment inspection, newest first. Powers the credibility
# log on the Shop console and the global view on /admin/equipment.
# ============================================================
...
# ============================================================
# Equipment Parts Catalog — per-unit wearable parts (filters,
# cutting edges, wiper blades, tires, other wear items) that
# field mechanics pull up before a PM service so they can order
# everything they need on the way out the door.
#
# Schema (per unit):
#   {
#     unit_number: str (primary key),
#     filters:          [ {name, part_number, qty, notes} ],
#     cutting_edges:    [ {name, part_number, qty, notes} ],
#     wiper_blades:     [ {name, part_number, size, qty, notes} ],
#     tires:            [ {position, size, ply, brand, part_number, qty, notes} ],
#     other_wear_items: [ {name, part_number, qty, notes} ],
#     updated_at: iso str,
#     updated_by: str (mechanic name)
#   }
# Mechanics + Admins can edit (require_shop_or_admin). Admin uploads
# an .xlsx/.csv to seed the whole catalog at once.
# ============================================================
...
async def upload_equipment_parts(
    ...
    (case-insensitive; spaces and dashes accepted: 'Cutting Edges' = 'cutting_edges').

    Each row appends a part to the corresponding category list for that unit.
    Existing parts for unmentioned units are left untouched.
    """
    raw = await file.read()
    fname = (file.filename or "").lower()
    rows: List[Dict[str, Any]] = []
    ...
    for unit, cats in by_unit.items():
        ...
        for c in PART_CATEGORIES:
            existing[c] = cats[c]  # replace category fully on upload
...
class PartsOrderRequest(BaseModel):
    unit_number: str
    equipment_label: Optional[str] = ""  # e.g. "Cat 320 Excavator"
    requested_by: str
    send_to: List[str]  # list of email addresses
...
async def email_parts_order(
    ...
    Used by field mechanics on their way to a PM service: pick the parts
    they need, hit one button, parts office has the order.
    """
    ...
    try:
        ...
        params = {
            "from": f"MASCI Shop <{sender_email}>",
            "to": payload.send_to,
