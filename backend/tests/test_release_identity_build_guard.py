...
def test_frontend_prebuild_invokes_build_guard() -> None:
    ...
    assert 'verifyGeneratedRuntimeContract' in src
...
def test_frontend_prebuild_passes_without_any_python_interpreter_on_path() -> None:
    node_path = shutil.which("node")
    assert node_path, "node is required for the stamp script regression tests"
    ...
    assert completed.returncode == 0
    assert "Python verifier unavailable" in completed.stderr