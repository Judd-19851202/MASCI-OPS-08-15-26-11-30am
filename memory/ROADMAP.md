...
| --------- | ----- | ---------------------------------------------------------------------------------------------------- | -------------------------------------------------------------- | --- | --------------------------------------- |
| **13.19** | A     | Proof-join (`operational_attachments` scale_ticket family) + virtual `verification_status` + rollup counters on existing `/api/material-movement/daily/{p}/{d}`. No new collection. No UI change. | `backend/routes/material_movement.py` (single file)            | 3h  | **NEXT — awaiting operator directive**  |
| 13.20     | B     | Read-only Material Movement panel on `PmProjectDetail.jsx` (project-scoped). Consumes Phase A.       | `frontend/src/pages/PmProjectDetail.jsx`                       | 2h  | Queued                                  |
| 13.21     | C     | Dispatch Companion Haul Ledger page + `/api/dispatch/haul-ledger` filter endpoint. Outside MapLibre. | new dispatch page + `backend/routes/dispatch_lifecycle.py` ext | 6h  | Queued                                  |
| 13.22     | D     | Admin Material Data-Quality page + CSV export + Admin Hub V2 card.                                   | new admin page + new endpoint                                  | 5h  | Queued                                  |
