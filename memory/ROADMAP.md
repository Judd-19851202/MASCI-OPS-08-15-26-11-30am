...
| **14.0-B1** | P1 · audit · pre-Spanish | Button audit — **1 385 instances** (corrected via A2: 934 shadcn `<Button>` + 451 native `<button>`) across 14 variants. Classify keep/consolidate/retire. **Author `BUTTONS_DICT.md`** documenting canonical English label per action concept. Translate dictionary, not 1 385 strings. | 4h | PENDING |
| **14.0-Mod1** | P1 · audit · pre-Spanish | Modal audit — 64 dialog/sheet/alert-dialog files · ~6 individually audited · 58 not. Per-modal Spanish + accessibility + mobile audit. Modal patterns must converge before per-string translation. | 4h | PENDING |
...
| **14.0-T1** | P3 · audit · pre-Spanish | Toast / terminology audit — **1 243 toast emissions** (corrected via A2 from A0's 1 440). Author `TOAST_DICTIONARY.md` + `TERMINOLOGY.md`. Polish `ViewIncident.jsx` HTTP-code-in-toast fallback. Translate dictionary, not 1 243 strings. | 6h | PENDING |
