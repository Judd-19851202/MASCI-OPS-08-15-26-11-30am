...
- Before opening new untouched routes, keep consuming the active ledger queues in order: close the 57 remaining `REPAIRED_NOT_CERTIFIED` rows, then clear the remaining shared admin search/export/add chrome defects and the documented blocker trio (`/admin/jha/:id`, `/ops-training/:slug`, `/admin/trench-safety-assets`).
