...
- **Trench Box QR Poster (2026-04-27):** Printable letter-size poster at `/admin/trench-boxes/poster` (admin-gated). Big MASCI-branded layout with a QR pointing to `https://mascidocs.com/trench-boxes`, soil-type quick reference (Type A / B / C with safety colors), and a fleet-snapshot table showing each shield's manufacturer/model/length/weight/Type C-60 max-depth. "Print Poster" button uses the same `printReport` helper as the Cheat Sheet (letter, 0.4" margins, color-exact). Bilingual (EN/ES) via the shared `useT()` toggle. Linked from `/admin/trench-boxes` header.
...
- **Bilingual coverage extended (2026-04-27):** Spanish dictionary expanded with all strings used by `JhaPlansHub`, `TrenchBoxes`, `Hub.jsx` (new tiles), and `TrenchBoxPoster`. Soil-type quick reference, fleet table headers, and CTA copy are all wrapped in `t(...)`. Read-only crew pages have no submission flow, so `translateOnSubmit` is not needed there — but every existing fillable form (Daily Reports, Inspections, Meetings, Incidents, Equipment) continues to translate freeform Spanish input back to English at POST time.
