...
| 2026-07-11 | 27.07 · Phase 0A+3+6 | R2 Storage Capacity Remediation — canonical extension only | ✅ **PHASE 0A+3+6 COMPLETE (preview)** — Track 28.12 duplicate module + tests removed; server.py mount removed. Small canonical extensions added **inside Track 27.06 files only**: `GET /api/admin/r2/lifecycle/sample` (break-the-classifier harness: 100 largest + N random + per-prefix + zero-byte + duplicate-ETag), `POST /api/admin/r2/lifecycle/quarantine` (logical mark only — R2 physical objects UNTOUCHED, holding window 24-720h default 7d), `GET /api/admin/r2/lifecycle/quarantine` (list), `POST /api/admin/r2/lifecycle/quarantine/cancel`. Only VERIFIED_ORPHAN is quarantine-eligible. `hard_delete_status: DISABLED` everywhere. 9/9 Track 27.07 storage invariants pass; 61 total in regression pass. Zero R2 mutation. Zero destructive endpoint enabled. **GO TO OPERATOR REVIEW** — awaits operator to (a) deploy to prod, (b) run `POST /api/admin/r2/lifecycle/scan`, (c) review classification + sample harness, (d) explicitly approve any quarantine batch. See `/app/memory/TRACK_27_07_PHASE_0_ARCHITECTURE_LOCK.md`. |
| 2026-07-11 | 27.07 · Phase 0 | R2 Lifecycle Governance & Scope Lock (architecture + ownership only) | ✅ **PHASE 0 COMPLETE** — canonical map produced; Track 27.06 confirmed as the one canonical R2 lifecycle architecture (`services/r2_lifecycle/*` + `routes/admin_r2_lifecycle.py` + `lib/r2_retention.py`). Track 28.12 fully classified — 3 duplicate endpoints marked REMOVE, 1 pattern REUSABLE only if legacy-cleanup track is chartered, 1 test OBSOLETE. Zero code changes. Zero deployment. Zero destructive operations. See `/app/memory/TRACK_27_07_PHASE_0_ARCHITECTURE_LOCK.md`. |
| 2026-07-11 | 28.11B | Final Track 28.11 production closeout (extended verification after ATT-28.11C-1 fix redeploy) | ✅ **PRODUCTION VERIFIED · CLOSED WITH PASS** — live prod `5bdf0f87316d`: System Health 8/8 HEALTHY, version card shows real ISO timestamp `built 2026-07-11T22:13:37.896607+00:00`, Deploy Readiness ATTENTION explicit, Governance overall_status=amber non-null, deployment ledger source_hash matches live, warning_classification present, OCC unique_critical_root_causes=2 with shared r2_bucket_capacity group. Zero Track 28.11 acceptance criterion depended on Track 28.12. |
| 2026-07-11 | 28.12 | Housekeeping + R2 forensics (parallel architecture) | 🟡 **UNAPPROVED DRAFT · SCOPE DRIFT** — endpoints exist on prod (from operator-authorized deploy) but not invoked. No R2 mutation. Preview env-var fix NOT redeployed per operator directive. See `/app/memory/TRACK_28_12_UNAPPROVED_DRAFT.md`. Awaits Track 27.06/27.07 to fold capability into canonical storage architecture or remove entirely. |
