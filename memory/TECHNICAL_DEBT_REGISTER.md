...
| TD-22.1c2-C01 | FastAPI `@app.on_event` deprecation — 51 startup + 1 shutdown handlers should migrate to `lifespan` context manager | **C** — Architecture Debt (STARTUP CLOSED) | Backend team | P2 | Tracks 22.1D · 22.1E · 22.1F · 22.1G · 22.1H · 22.1I · 22.1I.1 · 22.1J · 22.1L · 22.1K | **🎉 100% STARTUP MIGRATION COMPLETE** (2026-07-04 · Tracks 22.1D through 22.1L delivered · Track 22.1L retired the router-hosted `@router.on_event("startup")` closure in `routes/command_center.py` by registering `_command_center_seed_defaults` under `LIFECYCLE_STEPS.command-center` · 50/50 unique handlers migrated · on_startup 51 → 0 · migration progress live at `GET /api/admin/platform/status.migrated_pct` = **100.00%** · Only Track 22.1K (single `@app.on_event("shutdown")` handler) remains in the deprecation retirement program) |
