...
| TD-22.1c2-C01 | FastAPI `@app.on_event` deprecation — 51 startup + 1 shutdown handlers should migrate to `lifespan` context manager | **C** — Architecture Debt (foundation delivered) | Backend team | P2 | 22.1d + 22.1e/f/g/h/i/j/k | **FOUNDATION CLOSED · CLEANUP DEFERRED** (2026-07-04 · Track 22.1D delivered `lib/lifespan_bootstrap.py` orchestration wrapper + wired `FastAPI(lifespan=...)` · 207/207 lock envelope · zero handler moved · per-handler migration queued into 7 follow-up tracks with per-handler bytecode-fingerprint proof) |
