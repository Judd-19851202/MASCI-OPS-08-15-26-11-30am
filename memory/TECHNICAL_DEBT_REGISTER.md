...
| TD-22.1c2-C01 | FastAPI `@app.on_event` deprecation — 51 startup + 1 shutdown handlers should migrate to `lifespan` context manager | **C** — Architecture Debt (progressing) | Backend team | P2 | 22.1d + 22.1e/f/g/h/i/j/k | **FOUNDATION CLOSED · CUTOVER 43.14% COMPLETE** (2026-07-04 · Track 22.1D delivered `lib/lifespan_bootstrap.py` orchestration wrapper + wired `FastAPI(lifespan=...)` · Track 22.1E migrated 11 index-ensure handlers into `LIFECYCLE_STEPS.index-ensure` · Track 22.1F migrated 7 seed handlers into `LIFECYCLE_STEPS.seed` · Track 22.1G migrated 4 non-email schedulers into `LIFECYCLE_STEPS.scheduler-nonemail` — 22/51 total · on_startup count 51 → 29 · 246/246 lock envelope · 5 email-capable scheduler handlers quarantined for Track 22.1H · remaining 29 queued into Tracks 22.1H-K with per-track bytecode-fingerprint proof · migration progress live at `GET /api/admin/platform/status.migrated_pct`) |
