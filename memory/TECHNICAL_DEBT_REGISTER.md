...
| TD-22.1c2-C01 | FastAPI `@app.on_event` deprecation — 51 startup + 1 shutdown handlers should migrate to `lifespan` context manager | **C** — Architecture Debt (progressing) | Backend team | P2 | 22.1d + 22.1e/f/g/h/i/j/k | **FOUNDATION CLOSED · CUTOVER IN PROGRESS** (2026-07-04 · Track 22.1D delivered `lib/lifespan_bootstrap.py` orchestration wrapper + wired `FastAPI(lifespan=...)` · Track 22.1E migrated 11/51 index-ensure handlers into `LIFECYCLE_STEPS` registry via `@register_lifecycle_step("index-ensure")` — real cutover, no dual system for the migrated 11 · on_startup count 51 → 40 · 218/218 lock envelope · remaining 40 queued into Tracks 22.1F-K with per-track bytecode-fingerprint proof) |
