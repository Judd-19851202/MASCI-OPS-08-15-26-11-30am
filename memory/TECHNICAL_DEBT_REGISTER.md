...
| TD-22.1-C01 | `backend/server.py` = 16,094 lines. Modularization requires a full endpoint-parity harness (route set · Depends-chain · scheduler start-order · SDK-patch import ordering · startup/shutdown event count · health-endpoint bodies). Zero-Drift mandate blocks any move without parity proof. | **C** — Architecture Debt (deferred with plan) | Backend team | P1 | 22.1 | **PARTIAL — CLOSED for Phase 1** (2026-07-04 · Track 22.1 extracted health probes + rate-limiting to `backend/lib/*.py` with byte-comparable JSON parity proof · server.py 16,117 → 16,032 · full parity harness now permanent CI artifact · 0 endpoint / dependency-chain / startup-order drift · 162/162 lock envelope) |
| TD-22.1b-C01 | `_dispatch_auto_email` + Resend SDK monkey-patch import-ordering | **C** — Architecture Debt (deferred with plan) | Backend team | P2 | 22.1b | **OPEN — DEFERRED WITH PARITY GATE** (2026-07-04 · SDK patch must install before any router imports `resend`; extraction gate documented in `TRACK_22_1_MODULE_EXTRACTION_REPORT.md`) |
| TD-22.1c-C01 | Scheduler bootstrap (51 startup handlers, 39 create_task chains) | **C** — Architecture Debt (deferred with plan) | Backend team | P2 | 22.1c | **OPEN — DEFERRED WITH PARITY GATE** (2026-07-04 · startup-order parity gate available via `startup_handlers` snapshot) |
