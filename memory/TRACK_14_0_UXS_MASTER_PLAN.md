...
| **UXS-2b** | Admin / Shop / FL adoption of shared `<PortalShell>` | UXS-2 | YES | ⚠️ DEFERRED until UXS-11 — each shell already exposes full MASCI chrome parity; migration is purely structural with no user-visible benefit after UXS-2c. |
| **UXS-2c** | Full chrome on shared `<PortalShell>` (search + bell + switcher + sign out) | UXS-2 | YES | 🔴 **REJECTED 2026-06-14 — REOPENED.** Screenshots prove 6 failures remain: PM Command Center still purple (not on PortalShell), HR/Safety lane cards still show `Source: /api/...` captions, red MASCI preview strip stacks above Emergent's orange banner (double-banner), Dispatch Command chrome divergent, FL dead-button unverified. Shared shell upgrade I shipped is real but doesn't reach these surfaces. Next fork must: (1) delete the red preview strip from HrHubV2/PmHubV2/SafetyHubV2/DispatchHubV2, (2) migrate PmCommandCenter to PortalShell, (3) purge `Source: /api/...` from HR+Safety lane data, (4) wrap DispatchCommand in PortalShell, (5) post 8-route screenshot evidence. |
