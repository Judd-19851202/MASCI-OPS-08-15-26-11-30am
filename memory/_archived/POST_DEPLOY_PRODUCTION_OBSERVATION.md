...
| 🟢 INFO | Test data | Idempotency probe left one row in production `incidents` collection: `id=backup-forensics` (project_name=`PROD_MORNING_PROBE`, description=`morning verification — please delete`). | USER: Delete via `/admin → Incidents` list. | ❌ open |
