...
| iter394 | Dispatch Operational Board | ✅ shipped 2026-05-24 | `/dispatch-portal/board` route + `AssignmentDrawer` with full state_history timeline + 4 dispatcher actions (issue magic link · cancel · reassign · revoke session). 5 s polling. Tone-keyed state chips. Tenant-override aware (`?tenant=dls-demo`). |
| iter395 | Governance + Notifications + CSV | ✅ shipped 2026-05-24 | 4 detectors (`ASSIGNMENT_STUCK`, `WAIT_THRESHOLD_EXCEEDED`, `BREAKDOWN_ACTIVE`, `NON_STANDARD_TRANSITION_PATTERN`) + 3 CSV exports (assignments / state-events / haul-cycles) + calm FindingsBanner + ExportStrip on the board. 12 new tests, 48/48 platform total. Read-gated by `any portal token` so PM/Safety/Shop/FL hubs can light tiles in iter396+ without new endpoints. |
| iter396 | Coaching + Glossary + ES | ✅ shipped 2026-05-24 | 22 canonical DLS glossary entries (EN+ES) in `AdminOperationalLanguage.jsx`. ONE LifecycleGuide on the dispatch board ("What this board is telling you"). Shared `DispatchLifecycleTile` component mounted in PM Hub (project-scoped) + Shop Hub (BREAKDOWN-scoped). Backend gains optional `project_numbers` filter on `/governance/findings`. 3 NEW tests · 51/51 PASS total. |
