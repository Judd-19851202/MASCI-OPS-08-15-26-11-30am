# MASCI OPS — PLATFORM-WIDE TRUTH & TRUST RECONCILIATION — PROGRAM STATUS
System of record (resumable). Do NOT rely on chat history. Resume from this file + registers in this folder.

Production: https://mascidocs.com · Deployed release: 3dc83374c1c487c6d20a45cecd4791b7e1914444 (VERIFIED)
Preview candidate holds the release-identity root fix (NOT yet Saved — accumulate to a coherent checkpoint).
Final Product Quality v4 certification: PAUSED until this program completes + repaired release deployed & live-verified.

## MEASURED DENOMINATOR (do not silently shrink; expand on deeper discovery)
- Routed human-visible surfaces: 467
- KPI/count/health/status components: 547
- Population-shaping sites: 1,042 (frontend .slice(0,: 595 · backend to_list(N): 317 · backend limit=: 130)
- Hardcoded business totals found: 0 genuine (589-unit fleet already fixed+deployed)
...
- WAVE 7 Health R/Y/G failure-injection: NOT_STARTED
- WAVE 8 Cache/workers/fallbacks: NOT_STARTED
- WAVE 9 Portal-by-portal human truth: NOT_STARTED
- WAVE 10 Report/PDF/export/notification parity: NOT_STARTED
...
## RECONCILED LIVE (prior waves, evidence in registers)
- Employees 239 canonical; server-side q search full-population.
- Equipment 604 = 357 parts-eligible + 247 unnumbered; 4 malformed + 1 fixture (data-quality register).
- Jobs public lookup soft-delete parity (deleted_at filter) — live.
- Suppliers canonical server-side q search.

## WAVE 2 OPEN TRACING (next actions, no owner approval needed)
1. Exact 8 critical Recent Platform Events -> /api/admin/operations-control/events (paginate; enumerate id/ts/kind/severity/status/resolved; determine historical-vs-current aggregation).
2. Operations Control MISMATCH exact field derivation (platform/status VERIFIED + trust-reconciliation PASS/0 findings contradict card; trace OCC status source field).
3. Archive Storage ownership score 15.1 — find live card's actual endpoint (r2/lifecycle 404); trace formula/numerator/denominator.
4. Maintenance 5 attention ops + Deployment Maintenance UNKNOWN — find maintenance ops registry endpoint; enumerate op IDs.
5. Governance STALE->CRITICAL health contract — inspect derivation (critical=0,high=46,age~33d STALE); decide STALE must not render as active-critical.
6. Shared owners to audit: health/status color derivation (admin_operational_health._normalize_status), freshness, readiness/trust-spine, release identity (done), platform events aggregator (occ_trust_events), maintenance aggregation, storage lifecycle.

## SAVE CHECKPOINT STANDARD
Return to owner for Save only at a coherent checkpoint: all touched shared owners reconciled, no consumer regression, deterministic regressions added, responsive proof where affected, strict verifier clean (pre-save dirty expected), fingerprint deterministic x2, files/defects accounted.
