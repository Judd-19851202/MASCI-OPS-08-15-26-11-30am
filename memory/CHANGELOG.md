...
### CLIENT_UPDATE_REQUIRED contract + cache-policy audit (source-complete, PREVIEW)
- Backend `client_compat.py` + `_client_update_required_gate` middleware: incompatible client (governed `CLIENT_COMPAT_BLOCKLIST`, default EMPTY) → HTTP **426 CLIENT_UPDATE_REQUIRED** `{code,detail,client_release,action}`; ordinary/current clients untouched; `/api/health|/api/version|/api/auth|/api/deployment` exempt so a stale client can always update. Compatibility metadata only — never authorization. `/api/version` now advertises `client_compatibility` policy.
- Client: axios request interceptor sends `X-MASCI-Client-Release` (boot fingerprint from releaseUpdate); axios response interceptor absorbs 426 → `onClientUpdateRequired()` (protect dirty work → reload at safe boundary; reload immediately when clean). Banner adds UPDATE_REQUIRED copy.
- LIVE PROOF (temporary blocklist, then reverted): blocklisted release→426; no/other release→normal 401 (auth unchanged, NOT force-updated); /api/version→200 for blocklisted client. Reverted to accept-all.
- Cache-policy source audit: `frontend/public/_headers` now revalidates app bootstrap (`/`, `/index.html`, `/asset-manifest.json`, `/sw-thumbs.js` → no-cache, must-revalidate) while `/static/*` stays immutable; `/api/version` already `no-store` (verified); SW `sw-thumbs.js` confirmed scope-limited to `/api/job-photos/*/thumb` (cannot pin app shell) — left narrow.
- Gates: FE 68 suites/**434 tests**; backend compat **5/5**; IA **12/12**; release verifier ok=true (population/surface 0); deployable fingerprint ×2 identical = `dcf-2ec11d30375d5e0826392e5e56e4b3c582a5156a207d59e17595e9a8538dfe5e`. Save=NO · Deploy=NO · Production writes=0.
- POST-DEPLOY obligations (require real A→B prod transition): live A→B auto-update, live offline→reconnect convergence, live multi-tab convergence, actual deployed response-header audit (confirm host honours `_headers`).
