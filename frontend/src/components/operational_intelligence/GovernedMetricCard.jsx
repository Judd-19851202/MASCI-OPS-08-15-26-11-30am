  ...
  return (
    ...
    >
      ...
      <div className="mt-3 rounded-2xl bg-slate-50 p-3 text-xs text-slate-600" data-testid={`${testIdPrefix}-${metric.metric_id}-formula`}>
        <div className="font-semibold uppercase tracking-wider text-slate-500">Formula</div>
        <div className="mt-1 break-words">{metric.formula}</div>
      </div>

      <div className="mt-3 grid gap-2 text-xs text-slate-600 sm:grid-cols-2">
        <div data-testid={`${testIdPrefix}-${metric.metric_id}-freshness`}>
          <span className="font-semibold text-slate-800">Freshness:</span> {metric.freshness?.status || "—"} · {metric.freshness?.last_updated_at || metric.calculation_timestamp || "—"}
        </div>
        <div data-testid={`${testIdPrefix}-${metric.metric_id}-drilldown`}>
          <span className="font-semibold text-slate-800">Drill-down:</span> {metric.drilldown_path || "—"}
        </div>
        <div data-testid={`${testIdPrefix}-${metric.metric_id}-source-records`}>
          <span className="font-semibold text-slate-800">Source records:</span> {(metric.source_records || []).slice(0, 4).join(", ") || "—"}
        </div>
        <div data-testid={`${testIdPrefix}-${metric.metric_id}-work-blocks`}>
          <span className="font-semibold text-slate-800">Work Blocks:</span> {(metric.work_block_lineage || []).slice(0, 4).join(", ") || "—"}
        </div>
      </div>
