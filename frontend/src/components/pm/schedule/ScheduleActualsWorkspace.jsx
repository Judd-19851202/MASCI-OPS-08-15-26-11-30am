  ...
  return (
    <section className="rounded-[1.75rem] border border-white/30 bg-white/85 p-5 shadow-sm" data-testid="pm-project-schedule-actuals-section">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="text-xl font-black text-slate-900">{t("Schedule actuals review")}</h2>
          <p className="mt-1 text-sm text-slate-600">{t("Daily Reports remain the fact source. PM review is the authority gate before any schedule actual updates the current or forecast schedule.")}</p>
        </div>
          ...
          return (
            <div key={candidate.candidate_id} className="rounded-[1.5rem] border border-slate-200 bg-slate-50 p-4" data-testid={`pm-project-schedule-actual-card-${candidate.candidate_id}`}>
              <div className="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
                <div>
                  <div className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">{t("Candidate")}</div>
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <Badge variant={(candidate.activity_resolution?.confidence || "").includes("review") ? "outline" : "secondary"}>
                    {candidate.activity_resolution?.confidence || "review_required"}
                  </Badge>
                </div>
              </div>
              ...
              <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4 text-sm text-slate-700">
                <div className="rounded-2xl border border-slate-200 bg-white p-3" data-testid={`pm-project-schedule-actual-resolution-${candidate.candidate_id}`}>
                  <div className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">{t("Resolved activity")}</div>
                  <div className="mt-2 font-semibold text-slate-900">{candidate.activity_resolution?.resolved_activity_id || t("Needs PM review")}</div>
                  <div className="text-xs text-slate-500">{candidate.activity_resolution?.resolved_activity_name || t("No safe automatic match")}</div>
                </div>
                <div className="rounded-2xl border border-slate-200 bg-white p-3">
                  <div className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">{t("Installed actual")}</div>
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-3">
                  <div className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">{t("Registry linkages")}</div>
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-3">
                  <div className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">{t("Material flow")}</div>
                  ...
                  <div className="text-xs text-slate-600">{t("Outbound review")}: {(materialFlow.outbound_unclassified || []).length}</div>
                </div>
              </div>

              <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                <Input value={draft.activity_id ?? approved.activity_id ?? candidate.activity_resolution?.resolved_activity_id ?? ""} onChange={(event) => onCandidateDraft(candidate.candidate_id, "activity_id", event.target.value)} placeholder={t("Approved activity ID")} data-testid={`pm-project-schedule-actual-activity-id-${candidate.candidate_id}`} />
                <Input value={draft.activity_name ?? approved.activity_name ?? candidate.activity_resolution?.resolved_activity_name ?? ""} onChange={(event) => onCandidateDraft(candidate.candidate_id, "activity_name", event.target.value)} placeholder={t("Approved activity name")} data-testid={`pm-project-schedule-actual-activity-name-${candidate.candidate_id}`} />
                <Input type="number" step="0.01" value={draft.approved_percent_complete ?? approved.approved_percent_complete ?? 0} onChange={(event) => onCandidateDraft(candidate.candidate_id, "approved_percent_complete", event.target.value)} placeholder={t("Approved % complete")} data-testid={`pm-project-schedule-actual-percent-${candidate.candidate_id}`} />
                <Input type="number" step="0.01" value={draft.approved_installed_quantity ?? approved.approved_installed_quantity ?? candidate.actual_facts?.installed_quantity ?? 0} onChange={(event) => onCandidateDraft(candidate.candidate_id, "approved_installed_quantity", event.target.value)} placeholder={t("Approved installed qty")} data-testid={`pm-project-schedule-actual-quantity-${candidate.candidate_id}`} />
              ...
              <Textarea className="mt-3" value={draft.review_note ?? candidate.review_note ?? ""} onChange={(event) => onCandidateDraft(candidate.candidate_id, "review_note", event.target.value)} placeholder={t("PM review rationale, provenance notes, or why the candidate stays deferred.")} data-testid={`pm-project-schedule-actual-note-${candidate.candidate_id}`} />
              ...
              <div className="mt-4 flex flex-wrap justify-end gap-2">
                ...
                <Button type="button" onClick={() => onCandidateAction(candidate.candidate_id, "approve")} disabled={working} data-testid={`pm-project-schedule-actual-approve-${candidate.candidate_id}`}>{t("Approve actual")}</Button>
              </div>
            </div>
          );
        })}
        {candidates.length === 0 ? <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-4 text-sm text-slate-500" data-testid="pm-project-schedule-actuals-empty-state">{t("No schedule actual candidates yet. Submit a Daily Report with governed work blocks to start the review chain.")}</div> : null}
      </div>
    </section>
  );
};
