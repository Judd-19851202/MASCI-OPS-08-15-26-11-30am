// AdminFieldLeadershipUsersPanel — manage MASCI Field Leadership Portal accounts
// (Superintendent · Foreman · Truck Boss · Working Supervisor · Field Supervisor).
//
// iter314 · governed per-user identity, distinct from the legacy
// `/field-leadership/login` shared-password document gate. Both HR and
// Admin tokens are accepted by the backend route, so this panel is
// mounted in BOTH the AdminHub and the HR portal.
//
// Mirrors AdminShopUsersPanel/AdminHRUsersPanel pattern: list / add /
// edit / disable / delete + issue per-user passwords + resend welcome.
//
// Backend:
//   GET    /api/admin/field-leadership-users
//   POST   /api/admin/field-leadership-users         {name, email, phone?, role?, delivery, custom_password?}
//   PATCH  /api/admin/field-leadership-users/:id     (partial)
//   DELETE /api/admin/field-leadership-users/:id
//   POST   /api/admin/field-leadership-users/:id/reset-password   {delivery, custom_password?}
//   POST   /api/admin/field-leadership-users/:id/resend-welcome
//
// Field Leadership users sign in at /field-leadership/portal/login with their email + password.
...
export default function AdminFlUsersPanel() {
  ...
  const [form, setForm] = useState({ name: "", email: "", phone: "", role: "Superintendent" });
  ...
  const refresh = async () => {
    setLoading(true);
    try {
      const r = await api.get("/admin/field-leadership-users");
  ...
  const addUser = async () => {
    ...
    try {
      ...
      const r = await api.post("/admin/field-leadership-users", { ...form, delivery: "email" });
      toast.success(`HR user added — welcome email sent to ${r.data?.user?.email}`);
      setForm({ name: "", email: "", phone: "", role: "Superintendent" });
      refresh();
    } catch (err) {
  ...
  const startEdit = (u) => {
    setEditingId(u.id);
    setEditDraft({ name: u.name || "", email: u.email || "", phone: u.phone || "", role: u.role || "Superintendent" });
  };

  const saveEdit = async (u) => {
    setSavingId(u.id);
    try {
      await api.patch(`/admin/field-leadership-users/${u.id}`, editDraft);
      toast.success("Saved");
      setEditingId(null);
      refresh();
    } catch (err) {
  ...
  const toggleDisabled = async (u) => {
    setSavingId(u.id);
    try {
      await api.patch(`/admin/field-leadership-users/${u.id}`, { disabled: !u.disabled });
      refresh();
    } catch {
  ...
  const removeUser = async (u) => {
    ...
    try {
      await api.delete(`/admin/field-leadership-users/${u.id}`);
  ...
  const issueShowOnScreen = async (u) => {
    ...
    try {
      ...
      const r = await api.post(`/admin/field-leadership-users/${u.id}/reset-password`, payload);
  ...
  const issueEmail = async (u) => {
    ...
    try {
      ...
      if (custom) {
        await api.post(`/admin/field-leadership-users/${u.id}/reset-password`, { delivery: "custom", custom_password: custom });
        ...
        await api.post(`/admin/field-leadership-users/${u.id}/reset-password`, payload);
  ...
  return (
    <div className="border border-slate-200 rounded-md p-5 bg-white" data-testid="admin-field-leadership-users-panel">
      <div className="flex items-start justify-between gap-3 mb-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-600 mt-1 max-w-xl">
              ...
              sign in at <strong>/field-leadership/portal/login</strong> with their email + password
        ...
        <Button
          ...
          disabled={loading}
          className="h-9"
          data-testid="admin-field-leadership-users-refresh"
        >
      ...
      {/* Add user row */}
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 gap-x-6 gap-y-3 mb-4 p-3 bg-slate-50 rounded-md border border-slate-200">
                ...
                <tr key={u.id} className="border-t border-slate-100" data-testid={`admin-hr-row-${u.id}`}>
                  ...
                  <td className="px-3 py-2">
                    {isEditing ? (
                      ...
                      <span className="inline-block px-2 py-0.5 rounded bg-slate-100 text-slate-700 text-xs font-mono">{u.role || "Superintendent"}</span>
                    )}
                  </td>
                  <td className="px-3 py-2 text-slate-700">
                    {isEditing ? (
                  ...
                  <td className="px-3 py-2 text-right">
                    {isEditing ? (
                      ...
                      <div className="inline-flex gap-1">
                        <Button size="sm" variant="outline" onClick={() => startEdit(u)} className="h-8" title="Edit" data-testid={`admin-fl-edit-${u.id}`}><Pencil className="w-3.5 h-3.5" /></Button>
                        ...
                        <Button size="sm" variant="outline" onClick={() => removeUser(u)} className="border-red-300 text-red-700 hover:bg-red-50 h-8" title="Delete" data-testid={`admin-fl-delete-${u.id}`}><Trash2 className="w-3.5 h-3.5" /></Button>
                      </div>
                    )}
                  </td>
                </tr>
