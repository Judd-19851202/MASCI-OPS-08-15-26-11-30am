...
function DriverList({ drivers }) {
  ...
  return (
    <Card className="border-slate-200 shadow-sm" data-testid="forecast-driver-list">
      <CardHeader>
        <CardTitle className="text-lg text-slate-900">What is driving the outlook</CardTitle>
      </CardHeader>
      <CardContent className="space-y-3">
        {items.length === 0 ? <div className="text-sm text-slate-500">No main drivers were captured in this update.</div> : null}
        {items.map((driver, index) => (
          <div key={driver.driver_id || index} className="rounded-2xl border border-slate-200 bg-slate-50/80 p-4" data-testid={`forecast-driver-${index}`}>
            <div className="flex flex-wrap items-center gap-2">
              <div className="text-sm font-semibold text-slate-900">{driver.label || "Outlook driver"}</div>
              <Badge className={`border ${badgeTone(driver.family === "constraint" ? "at_risk" : "ready")}`}>{driver.family || "driver"}</Badge>
            </div>
            <div className="mt-2 text-sm text-slate-700">{driver.reason || "This condition is affecting the current outlook."}</div>
          </div>
        ))}
      </CardContent>
    </Card>
        ...
        <>
          ...
          <Tabs defaultValue="forecasting" className="space-y-5" data-testid="forecasting-tabs-root">
            <TabsList className="flex w-full flex-wrap justify-start gap-2 rounded-2xl bg-slate-100 p-1" data-testid="forecasting-tabs-list">
              ...
              <TabsTrigger value="governance" data-testid="governance-tab-trigger">History & source detail</TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="forecasting" className="space-y-5" data-testid="forecasting-tab-panel">
              ...
              <TableCard title="Scenario comparison" rows={(schedule?.scenario_comparison?.scenarios || []).map((row) => ({ id: row.scenario_key, scenario_label: row.scenario_label, notes: row.notes, projected_finish_date: row.projected_finish_date, days_gained_against_baseline: row.days_gained_against_baseline, days_lost_against_baseline: row.days_lost_against_baseline }))} testId="forecast-scenario-comparison-table" columns={[{ key: "scenario_label", label: "Scenario" }, { key: "notes", label: "Driver" }, { key: "projected_finish_date", label: "Projected finish" }, { key: "days_gained_against_baseline", label: "Days gained" }, { key: "days_lost_against_baseline", label: "Days lost" }]} emptyLabel="No scenario comparison rows are available for this saved update." />
              <TableCard title="Schedule pressure" rows={(schedule?.top_slipped_tasks || []).map((row) => ({ id: row.task_id || row.code, name: row.name || row.activity_name || row.code, slip_days: row.slip_days, forecast_finish_date: row.forecast_finish_date, explanation: row.explanation || row.reason || "Approved schedule output" }))} testId="forecast-schedule-pressure-table" columns={[{ key: "name", label: "Activity" }, { key: "slip_days", label: "Slip days" }, { key: "forecast_finish_date", label: "Forecast finish" }, { key: "explanation", label: "Why" }]} emptyLabel="No slipped schedule activities are currently preserved." />
            ...
            <TabsContent value="governance" className="space-y-5" data-testid="governance-tab-panel">
              <div className="grid gap-5 xl:grid-cols-[1.1fr_0.9fr]">
                <TableCard title="Recent forecast updates" rows={versions} testId="forecast-version-table" columns={[{ key: "version_number", label: "Update" }, { key: "generated_at", label: "Generated" }, { key: "note", label: "Note" }, { key: "change_detection", label: "Change", render: (row) => row.change_detection?.summary?.[0] || "No material change" }]} emptyLabel="No forecast updates have been preserved yet." />
              ...
              <div className="grid gap-5 xl:grid-cols-2">
                <TableCard title="Forecast vs field progress" rows={workspace?.forecast_vs_actual?.unit_rows || []} testId="forecast-vs-actual-table" columns={[{ key: "unit", label: "Unit" }, { key: "accepted_quantity", label: "Accepted", render: (row) => fmtNumber(row.accepted_quantity, 2) }, { key: "remaining_quantity", label: "Remaining", render: (row) => fmtNumber(row.remaining_quantity, 2) }, { key: "variance_reason", label: "Why" }]} />
                <Card className="border-slate-200 shadow-sm" data-testid="forecast-governance-card"><CardHeader><CardTitle className="text-lg text-slate-900">Where this outlook comes from</CardTitle></CardHeader><CardContent className="space-y-4 text-sm text-slate-700"><div data-testid="forecast-governance-authority">Schedule updates: <span className="font-semibold">{workspace?.authority_boundaries?.schedule_forecast_authority || "Approved schedule records"}</span></div><div data-testid="forecast-governance-production-authority">Production records: <span className="font-semibold">{workspace?.authority_boundaries?.production_authority || "Approved field production records"}</span></div><div data-testid="forecast-governance-manual-commitment-authority">Commitment records: <span className="font-semibold">{workspace?.authority_boundaries?.manual_commitment_authority || "Approved commitment records"}</span></div><div data-testid="forecast-governance-confidence">How dependable this outlook is: <Badge className={`border ${badgeTone(workspace?.confidence?.overall)}`}>{String(workspace?.confidence?.overall || "review_required").replaceAll("_", " ")}</Badge></div><div data-testid="forecast-governance-lineage">Record coverage: <Badge className={`border ${badgeTone(workspace?.confidence?.lineage_confidence)}`}>{String(workspace?.confidence?.lineage_confidence || "review_required").replaceAll("_", " ")}</Badge></div><div className="flex flex-wrap gap-3 pt-2"><Link to={`/pm/project-controls/schedule?project_number=${encodeURIComponent(projectNumber || "")}`} className="text-sm font-semibold text-teal-700 underline-offset-4 hover:underline" data-testid="forecast-governance-schedule-link">Open project schedule</Link>{mode !== "field" ? <Link to={`/pm/operational-intelligence?project_number=${encodeURIComponent(projectNumber || "")}`} className="text-sm font-semibold text-teal-700 underline-offset-4 hover:underline" data-testid="forecast-governance-ops-link">Open project performance</Link> : null}</div><div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-xs text-slate-600" data-testid="forecast-governance-note">Forecasts never auto-create commitments. Confidence widens when source records are sparse, record coverage is incomplete, or constraints remain unresolved.</div></CardContent></Card>
              </div>
            </TabsContent>
          </Tabs>
        </>
