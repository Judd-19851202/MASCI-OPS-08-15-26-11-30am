...
export default function GlobalSearch({
  ...
  const triggerBase = accent === "dark"
    ...
    : "bg-white hover:bg-zinc-50 border-zinc-300 text-zinc-800 placeholder:text-zinc-400";
  ...
  return (
    <>
      <button
        type="button"
        onClick={openOverlay}
        className={`inline-flex items-center gap-2 h-10 px-3 rounded-sm border text-sm font-semibold transition-colors wp16-focus-ring ${triggerBase} ${className}`}
      ...
      {open && (
        <div
          className="fixed inset-0 z-[80] flex items-start justify-center bg-black/35 backdrop-blur-[2px]"
        ...
        >
          <div
            className="mt-4 sm:mt-24 w-full sm:max-w-2xl mx-2 sm:mx-4 bg-white rounded-sm shadow-2xl border-2 border-zinc-900 max-h-[90vh] flex flex-col overflow-hidden"
            data-testid="global-search-panel"
          >
            <div className="flex items-center gap-2 px-3 py-3 border-b-2 border-zinc-900 bg-zinc-50">
              ...
              {!err && q.trim().length < 2 && recents.length > 0 && (
                <div className="p-3" data-testid="global-search-recents">
                  <div className="flex items-center justify-between mb-2">
                    <div className="text-[10px] font-mono uppercase tracking-[0.18em] text-zinc-500 font-bold flex items-center gap-1">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {recents.map((r) => (
                      <button
                        key={r}
                        className="text-xs px-2 py-1 rounded-sm border border-zinc-300 bg-zinc-50 hover:bg-zinc-100"
                        onClick={() => setQ(r)}
                ...
                <div className="divide-y divide-slate-100">
                  {data.groups.map((g) => (
                    ...
                    >
                      ...
                      <div className="text-[10px] font-mono uppercase tracking-[0.18em] text-zinc-700 font-bold">
                          {g.label}
                        </div>
                          ...
                          return (
                            <li key={`${row.kind}|${row.id}`}>
                              <button
                                ...
                                className={`w-full text-left px-3 py-2.5 flex items-center gap-3 hover:bg-zinc-50 focus:bg-zinc-50 focus:outline-none ${active ? "bg-zinc-50" : ""}`}
            ...
            {/* Footer hint */}
            <div className="px-3 py-2 border-t border-zinc-200 bg-zinc-50 text-[10px] font-mono uppercase tracking-[0.16em] text-zinc-500 flex items-center justify-between">
