...
export default function HrLogin() {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (res?.data?.ok && res?.data?.token) {
        ...
        // Track 15.14A Layer 2 — persist the flag so the route guard
        // catches deep-links / bookmarks too.
        setMustChange("hr", !!mustChange);
        if (mustChange) {
  ...
  return (
    ...
    >
      {/* iter322-B · context banner when redirected from a
          protected HR workflow. Zero footprint when absent. */}
      <AuthRequiredBanner />
