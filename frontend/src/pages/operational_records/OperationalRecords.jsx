      ...
      <div className="mx-auto max-w-6xl px-4 py-6">
        <header className="mb-6">
          <div className="text-xs uppercase tracking-wider text-slate-500">
            MASCI Operational Records
          </div>
          <h1 className="text-2xl font-semibold text-slate-900 mt-1">
            Operational Records
          </h1>
          <p className="mt-1 text-sm text-slate-600 max-w-2xl">
            Every project record across MASCI Ops in one place. New
            entries are filed as Operational Daily Records (ODR).
            Historical Daily Reports remain available, byte-identical
            to the day they were signed.
          </p>
        </header>
        ...
        <div className="mb-4 flex flex-wrap items-center gap-3">
          <input
            ...
            placeholder="Search doc id, project, foreman…"
          ...
          <div className="ml-auto text-xs text-slate-500" data-testid="op-records-counts">
            {counts.total} record(s) · {counts.odr} ODR · {counts.legacy_daily_report} archived
          </div>
        </div>
            ...
            {visibleItems.map((r) => (
              ...
              >
                ...
                <div className="mt-1 text-xs text-slate-500 flex flex-wrap gap-x-4 gap-y-1">
                  <span>Foreman: {r.foreman_name || "—"}</span>
                  {r.superintendent_name ? (
                    <span>Super: {r.superintendent_name}</span>
                  ) : null}
                  <span>Photos: {r.photo_count}</span>
                  {r.has_foreman_signature ? (
                    <span>Foreman signed</span>
                  ) : null}
                  {r.has_superintendent_signature ? (
                    <span>Super signed</span>
                  ) : null}
                  {r.archive ? (
