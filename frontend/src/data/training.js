...
const ADMIN_LESSONS = [
  ...
  },
  {
    ...
    duration: "~5 min",
    steps: [
      "Passwords live in env vars: ADMIN_PASSWORD, PM_PASSWORD (legacy), SHOP_PASSWORD (legacy). Per-PM and per-Shop-User accounts have their own bcrypt-hashed passwords stored in MongoDB.",
      "Frontend flow: login POST /api/{admin|pm|shop}/login → backend returns a 64-char HMAC token → frontend stores it in localStorage (or sessionStorage if Remember-me unchecked) and sends it as an X-{Admin|PM|Shop}-Token header on every request.",
      "Token has no expiry — it's invalidated by rotating the password OR bumping ADMIN_SESSION_EPOCH (which forces every active token of every role to re-auth at once).",
      "Rotate Admin: set ADMIN_PASSWORD to a new value in the production deploy env → redeploy. Every admin session is kicked.",
      ...
      "When a PM leaves: Admin → Project Managers → lock icon disables their account. Their token stops working immediately. Same for Shop users.",
    ],
    tips: [
      "If an admin password LEAKS: rotate immediately AND bump ADMIN_SESSION_EPOCH for instant kick of every role's tokens. Audit the activity_log collection for anything weird in the last 72 hours.",
      "ADMIN_HMAC_SECRET is the HMAC key that signs tokens. If THAT leaks, rotate it too — invalidates every admin/PM/shop session system-wide.",
    ],
    cheatSheet: [
      "Admin pw = env var. PM/Shop pw = per-user, hashed in DB.",
      "Rate limit: 10 fails → 15-min IP lockout.",
      "Bump ADMIN_SESSION_EPOCH for nuclear-option kick of every active token.",
    ],
  },
  {
