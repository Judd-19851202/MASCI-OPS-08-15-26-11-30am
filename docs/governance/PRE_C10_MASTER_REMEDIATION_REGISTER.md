...
Last updated: 2026-08-09T16:45Z
...
| PRE-C10-COACH-001 | Progressive disclosure / coaching | Optional workflow coaching was still competing with primary work on live routes, especially HR Employee Lifecycle, and several shared coaching patterns were not governed by one collapsed-by-default disclosure standard | PARTIAL PASS | shared primitive `frontend/src/components/WorkflowCoachingDisclosure.jsx` is repaired and targeted runtime evidence is green (`iteration_7.json` PASS; targeted screenshot coaching subset `20 / 20 PASS` at contract `wp18db-product-quality-v4`; `auto_frontend_testing_agent` PASS on `/hr/employees`, `/admin/daily`, `/dispatch-portal`, `/hr/historical-records/intake`, `/safety-portal/corrective-actions`; `deep_testing_backend_v2` `7 / 7 PASS`; targeted runtime ledger now also certifies `/operations-actions/:id` at `10 / 10 PASS`). However platform-wide coaching closure is still OPEN until the remaining inventoried login, lifecycle-guide, coaching-panel, OSHA-coaching, help-drawer, and other mapped route rows are explicitly runtime-dispositioned and the fresh full v4 ledger passes. |
...
| PRE-C10-ADMIN-003 | Admin OS information architecture / recovery truth | Owner-observed: `/admin` surfaced an oversized System Recovery block ahead of primary command information, and raw collection counts were framed like business truth. Root cause was shared hierarchy drift plus a legacy recovery panel mixing backup evidence, diagnostics, and destructive controls into the landing. Repair shipped: `/admin` is now an action-first command surface; recovery/storage/diagnostics are re-homed to canonical routes; `/admin/system` is exception-only; raw recovery counts are explicitly classified as technical diagnostics, not operator truth. Direct preview runtime verified on `/admin`, `/admin/system`, and `/admin/storage-recovery`; full EN/ES/responsive/final-ledger certification still remains in the active denominator. | PARTIAL PASS |
...
| PRE-C10-CROSS-ENTITY-001 | Cross-entity evidence & history integrity | New runtime audit surface shipped at `/api/admin/platform-truth-integrity/cross-entity` and is intentionally fail-closed. Shared repairs shipped in preview: new incident / daily-report / equipment-inspection writes now stamp canonical submitter IDs where available; employee/equipment master history now consumes meetings, daily reports, dispatch, and equipment inspections through governed links/bindings; preview backfill reduced equipment inspection exact-unit asset misses to `0`, added `9` operator employee links, and renormalized `76` meeting attendee sets. Remaining live blockers are still explicit: meeting MASCI name-only attendees (`38`), visible incident project/submitter lineage gaps, visible daily-report project/submitter lineage gaps, and dispatch truck/driver/project linkage drift. | OPEN / P0 |
...
5. Close `PRE-C10-CROSS-ENTITY-001` by proving canonical lineage or repairing the shared root cause for incidents, daily reports, dispatch, meetings, and remaining operator/history gaps before any final screenshot-ledger rerun.
