...
| PRE-C10-CROSS-ENTITY-001 | Cross-entity evidence & history integrity | **GREEN at the cross-entity gate, but not a PRE-C10 GO.** `/api/admin/platform-truth-integrity/cross-entity` now returns `overall_status=green` and `release_gate_blocked=false`. Shared repairs shipped in preview: deterministic canonical backfills for meetings/daily/equipment/dispatch where defensible, canonical submitter stamping on new incident/daily/equipment writes, employee/equipment history joins, and a governed Admin-only exception state/export for legitimate unresolved historical evidence. Current governed exception state: `9,800` active non-blocking rows (`accepted_historical_gap` / `excluded_non_operational`) and **0 blocking findings** at the cross-entity scanner. Verified by backend QA pass (30/30). | PASS at cross-entity gate / PRE-C10 overall still OPEN |
