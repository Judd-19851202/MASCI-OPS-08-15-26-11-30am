...
| Governance Summary | Persisted compliance findings inventory plus scan freshness/confidence | Severity/status/category/rule rollups + scan freshness SLA | `governance-trust` | `compliance_findings`, `compliance_scans` | `/api/admin/governance/summary` | Good at source; synthetic PPE/CAPA inflation defect repaired, daily-report employee-id backfill closed the missing-id class, and the current scan now expresses only live governance/business or real linkage debt conditions; broader platform KPI coverage still needs child linkage | Current within SLA | Good for current endpoint | `CURRENT` | Live governance summary formula | Governance / admin | Executive trust consumers via derived posture | Governance scan outputs | Local endpoint good; derivative-badge and downstream release classification still need reconciliation | Rule catalog + freshness present | 200; fresh scan `2026-08-10T09:59:24Z`, `open=58`, `resolved=1310`, `EMP_LINK_MISSING_ID=0`, `PPE_MISSING=0`, `INC_NEEDS_CAPA=0` after synthetic/applicability + daily-report backfill repair | FAIL |
...
| HR Time-Off Queue | FL-submitted time-off requests by decision status | Counts over `field_leadership_records` HR decision state | `hr-time-off` | `field_leadership_records` | `/api/field-leadership/time-off/stats` | Source-based evidence good; `/hr/time-off` now consumes governed KPI help, but downstream badge inventory still open | Current snapshot | Good | `CURRENT` | Current route metadata | HR / field leadership | Admin people-ops | Notifications / email / PDF still need KPI parity inventory | Endpoint and primary HR queue consumer now aligned; wider cross-surface parity still open | Pending count traceable to queue | 200; metadata present and runtime help rendered on `/hr/time-off` | FAIL |
...
| Operations Expirations Summary | Expiration posture across docs/training | Combined expiration-band counts across expiration sources | `expiration-governance` | `document_expirations`, `safety_training_records` | `/api/operations/expirations/summary` | Source-based evidence present; shared `ExpirationsSummary` and leadership consumer now render governed help, but full consumer inventory still open | Current snapshot | Good | `CURRENT` | Current route metadata | HR / Safety / Ops | Admin / executive operational health | Alerts / exports still need inventory | Shared component parity improved; every remaining badge/card still needs inventory proof | Endpoint metadata present | 200; metadata present and runtime help now renders in `ExpirationsSummary` + leadership compliance consumer | FAIL |
