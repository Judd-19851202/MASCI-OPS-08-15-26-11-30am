...
| PM Operational KPIs | `/api/pm/projects/OD-100/operational-kpis?window=ptd` | 200 | No top-level `kpi_metadata`; parity tests exist |
...
| PM Operational KPI family | Per-project labor/equipment/materials/production/delays/safety/intelligence posture | Shared operational KPI aggregator over canonical operational facts and safety sources | `operations-control` / project-controls domain | `operational_facts`, safety collections, supporting project sources | `/api/pm/projects/{project}/operational-kpis` | Partial; parity tests exist, but no top-level runtime metadata and not every consumer mapped | Current snapshot | Depends on source family completeness | `PARTIAL` | `contract_version=23.7` | PM project detail | Executive via downstream derived views | None fully inventoried yet | PM↔Safety subset parity tested | Drilldown partly available by section | 200 for `OD-100` | FAIL |
| Safety Project KPI family | Per-project safety-only subset from the shared operational KPI spine | Shared aggregator subset over governed project safety facts | `safety-truth` | same governed safety sources as PM operational family | `/api/safety/projects/{project}/safety-kpis` | Partial | Current snapshot | Good at endpoint payload; metadata gap remains | `PARTIAL` | Shared `23.7/23.8` KPI lineage | Safety project operators | Executive via derived safety posture | Safety reports still need inventory | PM↔Safety parity tests exist for subset | Drilldown partial | 200 for `OD-100` | FAIL |
